...
public class ActivitiesSaveRequestDto {
    ...
    @Builder
    public ActivitiesSaveRequestDto(String actName, String organizerPlan, String organizerManage,
                                     String tag, String startPeriod, String endPeriod, String contents,
                                     int price, String poster, String file, String manager, String page, String phone_number, int max_people) {
    ...
    public Activities toEntity() {
        return Activities.builder().actName(actName).organizerPlan(organizerPlan).organizerManage(organizerManage).tag(tag).startPeriod(startPeriod)
                .endPeriod(endPeriod).contents(contents).price(price).poster(poster).file(file).manager(manager).page(page).phone_number(phone_number).maxPeople(max_people).build();
    }
}
